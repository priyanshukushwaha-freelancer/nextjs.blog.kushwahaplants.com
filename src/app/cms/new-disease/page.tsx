...
import { createDiseaseAction } from '@/app/cms/actions';
import { ArrowLeft, Loader2 } from 'lucide-react';
import Link from 'next/link';

export default function NewDiseasePage() {
  const [state, formAction, isPending] = useActionState(createDiseaseAction, null);
  ...
  return (
    <div className="mx-auto w-full max-w-xl px-6 py-10 font-sans">
      ...
      <div className="space-y-1 mb-8">
        <h1 className="text-xl font-semibold tracking-tight text-[var(--foreground)]">Create Disease / Indication</h1>
        <p className="text-xs text-[var(--muted-foreground)]">Add a target medical indication category (e.g. Skin Infection).</p>
      </div>
      ...
      <form action={formAction} className="space-y-5">
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-[var(--foreground)]">Indication Name <span className="text-red-500">*</span></label>
          <input
            type="text"
            name="name"
            value={name}
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            rows={4}
            required
            placeholder="Describe the medical context and typical symptoms of this indication..."
        ...
        >
          {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <span>Create Indication</span>}
        </button>
      </form>
    </div>
  );
