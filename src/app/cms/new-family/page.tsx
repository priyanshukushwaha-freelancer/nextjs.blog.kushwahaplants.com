...
import { createFamilyAction } from '@/app/cms/actions';
  ...
  return (
    <div className="mx-auto w-full max-w-xl px-6 py-10 font-sans">
      ...
      <div className="space-y-1 mb-8">
        <h1 className="text-xl font-semibold tracking-tight text-[var(--foreground)]">Create Botanical Family</h1>
        <p className="text-xs text-[var(--muted-foreground)]">Add a taxonomic family category node (e.g. Lamiaceae).</p>
      </div>
      ...
      <form action={formAction} className="space-y-5">
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-[var(--foreground)]">Family Name <span className="text-red-500">*</span></label>
          <input
            type="text"
            name="name"
            value={name}
        ...
        <div className="space-y-1.5">
          ...
          <textarea
            ...
            rows={4}
            required
            placeholder="Describe the characteristics of this plant family classification..."
        ...
        >
          {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <span>Create Family</span>}
        </button>
      </form>
    </div>
  );
