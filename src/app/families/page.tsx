...
export default async function FamiliesIndexPage() {
  ...
  try {
    ...
    console.error('Error fetching botanical families:', error);
  }
    ...
    <div className="mx-auto w-full max-w-7xl px-6 lg:px-8 py-10 space-y-8 font-sans">
      <div>
        ...
        <p className="text-sm text-[var(--muted-foreground)] mt-1">
          Explore plants grouped by their scientific taxonomic family classifications.
        </p>
      </div>
      ...
      {families.length > 0 ? (
          ...
          {families.map((family) => (
            ...
            >
              ...
              <div className="flex items-center justify-between border-t border-[var(--border)]/45 pt-4 text-xs font-semibold text-[var(--ring)]">
                <span>{family._count?.plants || 0} Species Cataloged</span>
        ...
        <div className="text-center py-16 border border-dashed border-[var(--border)] rounded-2xl">
          <p className="text-sm text-[var(--muted-foreground)] italic">
            No botanical families indexed yet.
          </p>
        </div>
      )}
    </div>
