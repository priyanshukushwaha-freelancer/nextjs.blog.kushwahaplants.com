...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const action = await prisma.medicinalAction.findUnique({
  ...
  return {
    title: `${action.name} Plants - Kushwaha Plants`,
    description: `Scientific directory of plants possessing ${action.name} pharmacological properties. ${action.description}`,
  };
}
    ...
    <div className="mx-auto w-full max-w-7xl px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="space-y-4">
        <h2 className="font-sans text-lg font-semibold tracking-tight">
          Medicinal Species with this Action ({action.plants.length})
        </h2>

        {action.plants.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {action.plants.map(({ plant }) => (
              <Link
                key={plant.id}
          ...
          <p className="text-xs text-[var(--muted-foreground)] italic">
            No plants cataloged with this action yet.
          </p>
        )}
      </div>
    </div>
