...
import { createActionAction } from '@/app/cms/actions';
import { ArrowLeft, Loader2 } from 'lucide-react';
import Link from 'next/link';

export default function NewActionForm() {
  const [state, formAction, isPending] = useActionState(createActionAction, null);
  ...
  return (
    <div className="max-w-xl font-sans">
      ...
      <div className="space-y-1 mb-8">
        <h1 className="text-xl font-semibold tracking-tight text-[var(--foreground)]">Create Medicinal Action</h1>
        <p className="text-xs text-[var(--muted-foreground)]">Add a pharmacological or Ayurvedic action term (e.g. Adaptogenic, Anti-inflammatory, Immunomodulatory).</p>
      </div>
      ...
      <form action={formAction} className="space-y-5">
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-[var(--foreground)]">Action Name <span className="text-red-500">*</span></label>
          <input
            type="text"
            name="name"
            value={name}
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            required
            placeholder="adaptogenic"
            className="w-full px-4 py-2 text-sm rounded-xl border border-[var(--border)] bg-[var(--card)] focus:outline-none focus:border-[var(--ring)]"
          />
        </div>

        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-[var(--foreground)]">Description <span className="text-red-500">*</span></label>
          <textarea
            name="description"
            rows={3}
            required
            placeholder="Describe the therapeutic mechanism of action..."
        ...
        >
          {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <span>Create Medicinal Action</span>}
        </button>
      </form>
    </div>
  );
